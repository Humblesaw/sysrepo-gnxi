...
TEST_CASE("Top-level Set request (replace) empty", "[set]") {
  ...
  auto json = parent->printStr(DataFormat::JSON, PrintFlags::WithSiblings | PrintFlags::Shrink);
...
TEST_CASE("Top-level Set request leaflist (replace)", "[set]") {
  ...
  CHECK_THAT(json.value(), Equals("{\"gnmi-server-test:test4\":{\"params\":[\"mtu\",\"queue\",\"speed\"]}}"));
...
TEST_CASE("Top-level Set request inner leaflist (replace)", "[set]") {
  ...
  xpath_to_path("/gnmi-server-test:test/things[name=\"A\"]/amount-history", replace->mutable_path());
  replace->mutable_val()->set_json_ietf_val("[4,5,6]");
  auto status = client->Set(&ctx, request, &response);
  REQUIRE(status.ok());

  auto vals = sr_sess->getData("/gnmi-server-test:test/things[name=\"A\"]/amount-history");
  auto json = vals->printStr(DataFormat::JSON, PrintFlags::WithSiblings | PrintFlags::Shrink);
...
TEST_CASE("Top-level Set request leaflist (update)", "[set]") {
  ...
  CHECK_THAT(json.value(), Equals("{\"gnmi-server-test:test4\":{\"params\":[\"abc\",\"mtu\",\"queue\",\"speed\"]}}"));
  // Clean up
  sr_sess->deleteItem("/gnmi-server-test:test4");
  sr_sess->applyChanges();
}
...
TEST_CASE("Top-level Set request (update)", "[set]") {
  ...
  update->mutable_val()->set_json_ietf_val("{\"gnmi-server-test:test\":{\"things\":[{\"name\":\"A\",\"enabled\":true, \"ready\":[null]}]}}");
  auto status = client->Set(&ctx, request, &response);
  CHECK(status.ok());
...
TEST_CASE("Top-level Set request (replace)", "[set]") {
  ...
  xpath_to_path("/", replace->mutable_path());
  replace->mutable_val()->set_json_ietf_val("{\"gnmi-server-test:test\":{\"things\":[{\"name\":\"A\",\"enabled\":true}]}}");
  auto status = client->Set(&ctx, request, &response);
  CHECK(status.ok());
  ...
  // Item B should have been removed since the whole config was replaced and the new config only contained item A
  CHECK_THROWS_WITH(sr_sess->getOneNode("/gnmi-server-test:test/things[name='B']/enabled"), Contains("SR_ERR_NOT_FOUND"));
  // test2 should have been removed since the whole config was replaced and the new config includes only the test container
  CHECK_THROWS_WITH(sr_sess->getOneNode("/gnmi-server-test:test2/enabled"), Contains("SR_ERR_NOT_FOUND"));
...
TEST_CASE("Top-level Set request (multi replace)", "[set]") {
  ...
  xpath_to_path("/", replace->mutable_path());
  replace->mutable_val()->set_json_ietf_val("{\"gnmi-server-test:test\":{\"things\":[{\"name\":\"A\",\"enabled\":true}]},\"gnmi-server-test:test2\":{}}");
  auto status = client->Set(&ctx, request, &response);
  CHECK(status.ok());
  ...
  // Item B should have been removed since the whole config was replaced and the new config only contained item A
  CHECK_THROWS_WITH(sr_sess->getOneNode("/gnmi-server-test:test/things[name='B']/enabled"), Contains("SR_ERR_NOT_FOUND"));
  // test2 should have been removed since the whole config was replaced and the test2 container was empty in the new config
  CHECK_THROWS_WITH(sr_sess->getOneNode("/gnmi-server-test:test2/enabled"), Contains("SR_ERR_NOT_FOUND"));
...
TEST_CASE("Top-level Set request (multi replace + update)", "[set]") {
  ...
  xpath_to_path("/", replace->mutable_path());
  replace->mutable_val()->set_json_ietf_val("{\"gnmi-server-test:test\":{\"things\":[{\"name\":\"A\",\"enabled\":true}]},\"gnmi-server-test:test2\":{\"enabled2\":true}}");
  xpath_to_path("/", update->mutable_path());
  update->mutable_val()->set_json_ietf_val("{\"gnmi-server-test:test\":{\"things\":[{\"name\":\"A\",\"enabled\":false}]},\"gnmi-server-test:test2\":{\"enabled2\":false}}");
  auto status = client->Set(&ctx, request, &response);
  CHECK(status.ok());
...
TEST_CASE("Set request transaction (update)", "[set]") {
  ...
  xpath_to_path("/", update->mutable_path());
  update->mutable_val()->set_json_ietf_val("{\"gnmi-server-test:test\":{\"things\":[{\"name\":\"A\",\"enabled\":true}]}}");
  update = request.add_update();
  xpath_to_path("/", update->mutable_path());
  update->mutable_val()->set_json_ietf_val("{\"gnmi-server-test:test\":{\"things\":[{\"name\":\"B\",\"enabled\":true}]}}");
...
TEST_CASE("Set request transaction (delete+update)", "[set]") {
  ...
  CHECK_THAT(std::string(val.asTerm().valueStr()), Equals("true"));
  CHECK_THROWS_WITH(sr_sess->getOneNode("/gnmi-server-test:test/things[name='B']"), Contains("SR_ERR_NOT_FOUND"));
...
TEST_CASE("Set request (delete with wildcards)", "[set]") {
  ...
  CHECK_THROWS_WITH(sr_sess->getOneNode("/gnmi-server-test:test/things[name='A']/enabled"), Contains("SR_ERR_NOT_FOUND"));
  CHECK_THROWS_WITH(sr_sess->getOneNode("/gnmi-server-test:test/things[name='B']/enabled"), Contains("SR_ERR_NOT_FOUND"));
  sr_sess->deleteItem("/gnmi-server-test:test/things[name='A']");
  sr_sess->deleteItem("/gnmi-server-test:test/things[name='B']");
}
...
TEST_CASE("Set request (delete) with non-existent path", "[set]") {
  ...
  auto status = client->Set(&ctx, request, &response);
  CHECK(status.error_code() == StatusCode::OK);
...
TEST_CASE("Set request for list with composite key", "[set]") {
  ...
  sr_sess->switchDatastore(sysrepo::Datastore::Running);
  CHECK_THROWS_WITH(sr_sess->getOneNode("/gnmi-server-test:test3/complex-list[type='bar'][name='foo']/data"),
    Contains("SR_ERR_NOT_FOUND"));
  ...
  auto val = sr_sess->getOneNode("/gnmi-server-test:test3/complex-list[type='bar'][name='foo']/data");
...
std::string generate_random_string(size_t length) {
    std::string str;
    str.reserve(length); // Reserve space for the string to avoid reallocations
...
static size_t get_directory_size(const std::string dir) {
  using namespace std::filesystem;
  size_t size = 0;
  for (const auto& entry : recursive_directory_iterator(dir)) {
    if (!is_directory(entry)) {
      size += static_cast<size_t>(file_size(entry));
    }
  }
  return size;
}
...
TEST_CASE("Scaled Set request (update)", "[set-scale]") {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    for (int j = 0; j < 100; j++) {
      auto update = request.add_update();
      auto xpath = "/gnmi-server-test:test/things[name=\"A" + std::to_string(j) + "\"]/description";
  ...
  SECTION("Check file archive and log size") {
    auto logsize = get_directory_size(GNMI_LOG_DIR "/raw");
    CHECK(logsize < 2 * 1024 * 1024);

    auto archive_size = get_directory_size(GNMI_LOG_DIR "/archives");
    CHECK(archive_size < 8 * 1024 * 1024);
  }
}
...
TEST_CASE("Set request (ascii val type)", "[set-neg]") {
  ...
  auto status = client->Set(&ctx, request, &response);
  CHECK(status.error_code() == StatusCode::UNIMPLEMENTED);
...
TEST_CASE("Set request (incorrect prefix)", "[set-neg]") {
  ...
  auto status = client->Set(&ctx, request, &response);
  CHECK(status.error_code() == StatusCode::INVALID_ARGUMENT);
  CHECK_THAT(status.error_message(), Equals("Node \"enabled\" not found as a child of \"things\" node. (Line number 1.)"));
...
TEST_CASE("Set request failing transaction (2 updates)", "[set-neg]") {
  ...
  update->mutable_val()->set_json_ietf_val("{\"gnmi-server-test:test\":{\"things\":[{\"name\":\"B\",\"enabled\":\"maybe\"}]}}");

  auto status = client->Set(&ctx, request, &response);
  CHECK(status.error_code() == StatusCode::INVALID_ARGUMENT);
  CHECK_THAT(status.error_message(), Equals("Invalid non-boolean-encoded boolean value \"maybe\". (Data location \"/gnmi-server-test:test/things[name='B']/enabled\", line number 1.)"));
  ...
  CHECK_THROWS_WITH(sr_sess->getOneNode("/gnmi-server-test:test/things[name='A']"), Contains("SR_ERR_NOT_FOUND"));
  CHECK_THROWS_WITH(sr_sess->getOneNode("/gnmi-server-test:test/things[name='B']"), Contains("SR_ERR_NOT_FOUND"));
  ...
  update = request.add_update();
  xpath_to_path("/", update->mutable_path());
  update->mutable_val()->set_json_ietf_val("{\"gnmi-server-test:test\":{\"things\":[{\"name\":\"C\",\"enabled\":true}]}}");
...
TEST_CASE("Set request failing transaction (delete+update)", "[set-neg]") {
  ...
  xpath_to_path("/", update->mutable_path());
  // Contains bad value for enabled so expected to fail
  update->mutable_val()->set_json_ietf_val("{\"gnmi-server-test:test\":{\"things\":[{\"name\":\"A\",\"enabled\":\"maybe\"}]}}");

  auto status = client->Set(&ctx, request, &response);
  CHECK(status.error_code() == StatusCode::INVALID_ARGUMENT);
  CHECK_THAT(status.error_message(), Equals("Invalid non-boolean-encoded boolean value \"maybe\". (Data location \"/gnmi-server-test:test/things[name='A']/enabled\", line number 1.)"));
...
TEST_CASE("Set request failing transaction (2 leaf updates)", "[set-neg]") {
  ...
  auto status = client->Set(&ctx, request, &response);
  CHECK(status.error_code() == StatusCode::INVALID_ARGUMENT);
  CHECK_THAT(status.error_message(), Equals("Invalid boolean value \"maybe\". (Schema location \"/gnmi-server-test:test/things/enabled\".)"));
...
TEST_CASE("Top-level Set request (update, no namespace)", "[set-neg]") {
  ...
  xpath_to_path("/", update->mutable_path());
  update->mutable_val()->set_json_ietf_val("{\"test\":{\"things\":[{\"name\":\"A\",\"enabled\":true}]}}");
  auto status = client->Set(&ctx, request, &response);
  CHECK(status.error_code() == StatusCode::INVALID_ARGUMENT);
  CHECK_THAT(status.error_message(), Equals("Top-level JSON object member \"test\" must be namespace-qualified. (Line number 1.)"));
...
TEST_CASE("Set request (update with wildcards)", "[set-neg]") {
  ...
  CHECK_THAT(status.error_message(),
	Equals("Couldn't find schema node: /gnmi-server-test:test/*/enabled"));
}
...
TEST_CASE("Set request (application error string)", "[set-neg]") {
  ...
  CHECK_THAT(status.error_message(), Equals("Fiddlesticks: /gnmi-server-test:test2/custom-error"));
}
...
TEST_CASE("Set request (data model error)", "[set-neg]") {
  ...
  auto status = client->Set(&ctx, request, &response);
  CHECK(status.error_code() == StatusCode::ABORTED);
  CHECK_THAT(status.error_message(), Equals("Must condition \"current() > 42\" not satisfied. (Data location \"/gnmi-server-test:test2/must-error\".)"));
}
