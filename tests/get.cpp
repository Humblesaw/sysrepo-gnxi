...
// A path referring to "root" (which is represented by a path consisting of an empty set of elements) should result in the nodes childA and childB and all of their children ... being considered by the relevant operation.
TEST_CASE("Top-level Get request", "[get]") {
  ...
  for (auto it : response.notification().Get(0).update()) {
    ...
    if (path.compare("/gnmi-server-test:test-state"))
      continue;
    found = true;
    CHECK(response.notification().Get(0).update().Get(0).val().value_case() == gnmi::TypedValue::ValueCase::kJsonIetfVal);
...
TEST_CASE("Get request of all module oper state", "[get]") {
  ...
  CHECK_THAT(json, Equals("{\"things\":[{\"name\":\"A\",\"counter\":\"1\"},{\"name\":\"B\",\"counter\":\"2\"}],\"cargo\":{}}"));
...
// gNMI spec reference:
// In the case that the data item at the specified path is a leaf node (i.e., has no children, and an associated value) the value of that leaf is encoded directly - i.e., the "bare" value is specified (i.e., a JSON object is not CHECKd, and a bare JSON value is included).
...
TEST_CASE("Get request with wildcard", "[get]") {
  ...
  auto path = path_to_xpath(response.notification().Get(0).update().Get(0).path());
  CHECK_THAT(path, Equals("/gnmi-server-test:test-state/things[name='A']/counter"));
  CHECK(response.notification().Get(0).update().Get(1).val().value_case() == gnmi::TypedValue::ValueCase::kJsonIetfVal);
...
TEST_CASE("Get request with multiple paths", "[get]") {
  ...
  CHECK(response.notification().Get(1).update().Get(0).val().value_case() == gnmi::TypedValue::ValueCase::kJsonIetfVal);
...
TEST_CASE("Get request of one list item where name contains /", "[get]") {
  ...
  sr_sess->setItem("/gnmi-server-test:test-state/things[name='Gigabit5/0/0']",
			std::nullopt);
...
TEST_CASE("Get request of one leaf where parent name contains /", "[get]") {
  ...
  const Path reqpath = request.path(0);
...
TEST_CASE("Get request of one list item where name contains [", "[get]") {
  ...
  sr_sess->switchDatastore(sysrepo::Datastore::Operational);
  sr_sess->setItem("/gnmi-server-test:test-state/things[name='One[1]']",
			std::nullopt);
...
TEST_CASE("Get request of one list item where name contains '", "[get]") {
  ...
  sr_sess->switchDatastore(sysrepo::Datastore::Operational);
  sr_sess->setItem("/gnmi-server-test:test-state/things[name=\"to-cpe2'\"]",
			std::nullopt);
...
TEST_CASE("Get request from list with composite key ", "[get-composite-key]") {
  ...
  sr_sess->switchDatastore(sysrepo::Datastore::Operational);
  sr_sess->setItem("/gnmi-server-test:test3/complex-list[type='bar'][name='foo']/data",
			"baz");
  sr_sess->applyChanges();
...
TEST_CASE("Get request from list with composite key having slashes ", "[get-composite-key-with-slashes]") {
  ClientContext ctx;
  GetRequest request;
  GetResponse response;
  ...
  // create an entry in the list wiht composite keys
  sr_sess->switchDatastore(sysrepo::Datastore::Operational);
  sr_sess->setItem("/gnmi-server-test:test3/complex-list[type='bar/cat/fish'][name='foo/dog/sausage']/data",
            "baz");
  sr_sess->applyChanges();

  // prepare a get request for that entry
  xpath_to_path("/gnmi-server-test:test3/complex-list[name='foo/dog/sausage'][type='bar/cat/fish']",
        request.add_path());
  ...
  CHECK(response.notification().Get(0).update().Get(0).val().value_case() == gnmi::TypedValue::ValueCase::kJsonIetfVal);
  auto json = response.notification().Get(0).update().Get(0).val().json_ietf_val();
  CHECK_THAT(json, Equals("{\"type\":\"bar/cat/fish\",\"name\":\"foo/dog/sausage\",\"data\":\"baz\"}"));
  auto resppath = response.notification().Get(0).update().Get(0).path();
  CHECK(resppath.elem_size() == 2);
  CHECK_THAT(resppath.elem(0).name(), Equals("gnmi-server-test:test3"));
  CHECK_THAT(resppath.elem(1).name(),  Equals("complex-list"));
  ...
  // cleanup
  sr_sess->deleteItem("/gnmi-server-test:test3/complex-list[type='bar/cat/fish'][name='foo/dog/sausage']");
  sr_sess->applyChanges();
}
TEST_CASE("Get request from list with composite key having doube-quotes(\") ", "[get-composite-key-with-quotes]") {
  ClientContext ctx;
  GetRequest request;
  GetResponse response;
  ...
  sr_sess->switchDatastore(sysrepo::Datastore::Operational);
  sr_sess->setItem("/gnmi-server-test:test3/complex-list[type='bar/cat/fish'][name='quotes: \", blah blah']/data",
            "baz");
  sr_sess->applyChanges();

  // prepare a get request for that entry
  xpath_to_path("/gnmi-server-test:test3/complex-list[name='quotes: \", blah blah'][type='bar/cat/fish']",
		request.add_path());
  ...
  CHECK(response.notification().Get(0).update().Get(0).val().value_case() == gnmi::TypedValue::ValueCase::kJsonIetfVal);
  auto json = response.notification().Get(0).update().Get(0).val().json_ietf_val();
  CHECK_THAT( json, Equals("{\"type\":\"bar/cat/fish\",\"name\":\"quotes: \\\", blah blah\",\"data\":\"baz\"}") );
  auto resppath = response.notification().Get(0).update().Get(0).path();
  CHECK(resppath.elem_size() == 2);
  CHECK_THAT(resppath.elem(0).name(), Equals("gnmi-server-test:test3"));
  CHECK_THAT(resppath.elem(1).name(),  Equals("complex-list"));
  ...
  // cleanup
  sr_sess->deleteItem("/gnmi-server-test:test3/complex-list[type='bar/cat/fish'][name='quotes: \", blah blah']");
  sr_sess->applyChanges();
}
