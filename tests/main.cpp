...
unique_ptr<gNMI::Stub> client;
std::optional<sysrepo::Session> sr_sess;

// Parse XPath-like string in gnmi::Path
void
xpath_to_path(std::string xpath, gnmi::Path *path)
{
...
std::string
path_to_xpath(const gnmi::Path &path)
{
  string str = "";
...
module_change_cb(sysrepo::Session session, uint32_t sub_id, std::string_view module_name, std::optional<std::string_view> xpath, sysrepo::Event event, uint32_t request_id)
{
  (void)session;
  (void)module_name;
  (void)request_id;
...
static sysrepo::ErrorCode
clear_stats_rpc_cb(
		sysrepo::Session session, uint32_t sub_id, std::string_view xpath, const libyang::DataNode input, sysrepo::Event event, uint32_t request_id, libyang::DataNode output)
{
  (void)session;
  (void)event;
  (void)request_id;
...
public:
  SetupSysrepo() {
    ...
    sr_conn.installModules(
      {
        TESTS_SRC_DIR "/gnmi-server-test.yang",
        TESTS_SRC_DIR "/gnmi-server-test-wine.yang"
      }, TESTS_SRC_DIR);

    sr_sess = sr_conn.sessionStart();

    sub = sr_sess->onModuleChange("gnmi-server-test", module_change_cb, "/gnmi-server-test:test2/custom-error");
    ...
    sub->onRPCAction(
          "/gnmi-server-test:clear-stats", clear_stats_rpc_cb,
                  0,
                  sysrepo::SubscribeOptions::Default);
  };

  ~SetupSysrepo() {
    BOOST_LOG_TRIVIAL(debug) << "Removing sysrepo data" << std::endl;
    sub.reset();
...
public:
  ~SetupTests() {
    BOOST_LOG_TRIVIAL(debug) << "Removing test directories" << std::endl;
    fs::remove_all(log_path);
    if (!repo_path.empty()) {
      fs::remove_all(repo_path);
      fs::remove_all(sr_shm_path);
    }
    fs::remove("/tmp/gnmi-logrotate.lock");
  }

  SetupTests() {
    log_path = string(GNMI_LOG_DIR);
    fs::remove_all(log_path);
    fs::remove("/tmp/gnmi-logrotate.lock");
    fs::create_directory(log_path);
    fs::create_directory(log_path + "/raw");
    fs::create_directory(log_path + "/archives");
    ...
    if (getenv("SYSREPO_REPOSITORY_PATH")) {
      ...
      BOOST_LOG_TRIVIAL(warning) << "Using user-defined sysrepo env" << std::endl << std::endl;
      return;
    }
    ...
    if (fs::exists(sr_shm_path)) {
      fs::remove_all(sr_shm_path);
    }

    if (setenv("SYSREPO_REPOSITORY_PATH", repo_path.c_str(), 0)) {
      throw std::runtime_error("Failed to setenv SYSREPO_REPOSITORY_PATH " + std::string(strerror(errno)) );
    }

    if (setenv("SYSREPO_SHM_DIR", sr_shm_path.c_str(), 0)) {
      throw std::runtime_error("Failed to setenv SYSREPO_SHM_DIR " + std::string(strerror(errno)) );
    }

    if (setenv("SR_ENV_RUN_TESTS", "1", 0)) {
      throw std::runtime_error("Failed to setenv SR_ENV_RUN_TESTS " + std::string(strerror(errno)) );
    }

    // Print this so debugging is easier
    BOOST_LOG_TRIVIAL(info) << "Running tests with " << std::endl
      << "SYSREPO_REPOSITORY_PATH=" << repo_path << std::endl
      << "SYSREPO_SHM_DIR=" << sr_shm_path << std::endl
      << "SR_ENV_RUN_TESTS=1" << std::endl;
  }
...
int main(int argc, char* argv[])
{
  ...
  string bind_addr = "localhost:40051";
  Log().setLevel(4);
  ...
  std::thread server_thread(RunServer, bind_addr, auth.build(), sysrepo::Connection(), std::move(promise));
